  ...
  return (
    <group ref={orbitRef}>
      ...
      <Html position={[planet.distance, planet.size + 0.5, 0]} center>
        <motion.div 
          className={`px-2 py-1 rounded text-xs font-mono-lab whitespace-nowrap transition-all cursor-pointer ${
            isSelected ? 'bg-neon-cyan/40 text-neon-cyan scale-110' : 'bg-black/60 text-white/80 hover:bg-black/80'
          }`}
          onClick={onSelect}
              ...
              {planets.map((planet, index) => (
                <motion.button
                  ...
                  className={`w-4 h-4 rounded-full transition-all ${
                    selectedPlanet === index ? 'ring-2 ring-neon-cyan ring-offset-2 ring-offset-background scale-125' : 'hover:scale-110'
                  }`}
            ...
            <div className="flex gap-2">
              {[0.25, 0.5, 1, 2, 4].map((scale) => (
                <motion.button
                  ...
                  className={`px-2 py-1 rounded text-xs font-mono-lab transition-all ${
                    timeScale === scale 
